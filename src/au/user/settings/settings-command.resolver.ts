...
export class SettingsCommandResolver {
  ...
  async accept2fa(@CurrentUserId() id: string) {
    return this.commandBus.execute<Accept2faCommand, AcceptType>(
      new Accept2faCommand(id),
    );
  }
  ...
  async verify2fa(@Args('code') code: string, @CurrentUserId() id: string) {
    return await this.commandBus.execute<Verify2faCommand, StatusType>(
      new Verify2faCommand(id, code),
    );
  }
}
