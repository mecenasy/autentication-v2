...
import { Location } from 'src/common/geo/geo.service';
import {
  RISK_PROXY_SERVICE_NAME,
  RiskProxyServiceClient,
} from 'src/proto/risk';
...
export class RiskService implements OnModuleInit {
  ...
  onModuleInit() {
    this.grpcService = this.grpcClient.getService<RiskProxyServiceClient>(
      RISK_PROXY_SERVICE_NAME,
    );
  }
  ...
  private checkLoginLock(history: History) {
    ...
    const lockUntil = new Date(
      new Date(history.lastFailureAt).getTime() + duration * 60000,
    );
  ...
  private isSuspiciousUserAgent(
    ...
    if (
      current.device.model === 'iPhone' &&
      current.browser.name !== 'Mobile Safari'
    ) {
      return true;
    }
