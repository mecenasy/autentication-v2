...
export class GetPasskeysHandler extends Handler<
  GetPasskeysQuery,
  any,
  PasskeyProxyServiceClient
> {
  ...
  async execute({ userId }: GetPasskeysQuery) {
    let passkeys = await this.cache.getFromCache<
      GetPasskeysResponse['passkeys']
    >({
      identifier: userId,
      prefix: 'passkeys',
    });
    ...
    passkeys = (await lastValueFrom(this.gRpcService.getPasskeys({ userId })))
      .passkeys;
