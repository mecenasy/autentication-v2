...
import { LoginStatusType } from 'src/autemticator/auth/login/dto/login-status.tape';
...
> {
  ...
  async execute({
    id,
  }: AcceptAdaptiveLoginCommand): Promise<AcceptAdaptiveLoginType> {
    const { active } = await lastValueFrom(
      this.gRpcService.acceptAdaptive({ id }),
    );
