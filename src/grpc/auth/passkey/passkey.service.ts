...
export class PasskeyService {
  ...
  async addPasskey({
    ...
    publicKey,
    userId,
  }: AddPasskeyRequest): Promise<PasskeyResponse> {
  ...
  async getPasskey({
    credentialID,
  }: GetPasskeyRequest): Promise<GetPasskeyResponse> {
  ...
  async getPasskeys({
    userId,
  }: GetPasskeysRequest): Promise<GetPasskeysResponse> {
    const passkeys = await this.passkeyRepository.find({
      where: { user: { id: userId } },
    });

    return {
      passkeys: passkeys.map<GetPasskeysResponse['passkeys'][number]>(
        (passkey) => ({
          id: passkey.id,
  ...
  async setCounter({
    credentialID,
    counter,
  }: SetCounterRequest): Promise<PasskeyResponse> {
